...
class ViewsTests(TestCase):
    ...
    def fields_post_correct(self, response_post):
        ...
        value_fields_post = (
            ...
            # Добавить поле проверки картинки
        )
    ...
    def test_correct_template_not_public_urls(self):
        ...
        for url_name, template_url in ViewsTests.name_urls_not_public_template:
            with self.subTest(url_name=url_name):
                ...
                self.assertTemplateUsed(response, template_url)
# Проверить контекст поста с картинкой здесь
...
# Проверить контекст поста с картинкой здесь
    ...
    def test_comments_autorized_user(self):
        ...
        self.autorized_user_client.post(
            self.name_url_comments,
            comment_text
        )
        response = self.autorized_user_client.get(name_url_detail_add_comment)
        response_comment_text = response.context['comments'].get(
            text=comment_text['text']).text
        self.assertEqual(
    ...
    def test_cache_index(self):
        ...
        response_after_removal = self.client.get(
            self.name_urls_public_template[0][0])
    ...
    def test_follow_authorized_user(self):
        ...
        self.autorized_user_client.get(ViewsTests.name_url_follow[0][0])
        count_follow = Follow.objects.count()
        for url_follow, follow in ViewsTests.name_url_follow:
            with self.subTest(url_follow=url_follow):
                self.autorized_user_client.get(url_follow)
                self.assertEqual(
    ...
    def test_post_add_in_follow(self):
        ...
        response = self.follower_user_client.get(
